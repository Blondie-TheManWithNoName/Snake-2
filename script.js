...
class Ball {
  constructor(pos, id) {
    if (id <= 65) this.pos = { x: pos.x, y: pos.y };
...
class Snake {
  constructor(length, iniPos, clicked, color) {
    ...
    for (let i = 0; i < length; ++i) this.snake.push(new Ball(iniPos, i));
  }
  ...
  calculateMovement() {
    if (this.turn) {
      ...
      let targetDir = {
        x: this.clicked.x - this.snake[0].pos.x,
        y: this.clicked.y - this.snake[0].pos.y,
      };
      let currentDir = {
        x: this.snake[0].pos.x - this.snake[1].pos.x,
        y: this.snake[0].pos.y - this.snake[1].pos.y,
      };
  ...
  paintClick() {
    ...
    ctx.arc(
      this.clicked.x,
      this.clicked.y,
      radius - 0.1 * clickAnimation,
      0,
      Math.PI * 2
    );
    ctx.stroke();

    if (clickAnimation >= 50) clickAnimation = 0;
    ++clickAnimation;
  }
  ...
  move() {
    ...
    if (
      Math.abs(Math.abs(this.snake[0].pos.x) - Math.abs(this.clicked.x)) <
        speed &&
      Math.abs(Math.abs(this.snake[0].pos.y) - Math.abs(this.clicked.y)) < speed
    )
      this.reached = true;
  }
  ...
  eat() {
    if (this.collision(apples[0])) {
      let copyScore = (++score).toString();
      console.log(copyScore);
      for (let i = 0; i < copyScore.length; ++i) {
        let element = document.getElementsByClassName("score");

        if (element[i]) element[i].textContent = copyScore.charAt(i);
        else {
          var newScoreElement = document.createElement("div");
          newScoreElement.className = "score";
          newScoreElement.textContent = copyScore.charAt(i);
          document.getElementById("bar").appendChild(newScoreElement);
        }
  ...
  addTail(num) {
    for (let i = 0; i < num; ++i)
      this.snake.push(
        new Ball(this.snake[this.snake.length - 1].pos, this.snake.length - 1)
      );
  }

  paint() {
    for (const ball of this.snake) ball.paint("#BF212F");
  }
  ...
  selfBite() {
    if (this.snake.length > 65) {
      ...
      for (let i = 66; i < this.snake.length; i++) {
        ...
        if (this.collision(bodyPart.pos)) return true;
      }
    }
  ...
  collision(pos) {
    ...
    return (
      directionX * directionX + directionY * directionY < radius * radius * 4
    );
  }
...
function draw() {
  ...
  generateApple();
  snake.move();
  if (snake.eat()) snakeBlue.addTail(5);
  ...
  if (
    ...
    yOut(snake.snake[0].pos.y) ||
    snake.selfBite() ||
    xOut(snakeBlue.snake[0].pos.x) ||
    yOut(snakeBlue.snake[0].pos.y) ||
    snakeBlue.selfBite() ||
    biteOther(snake, snakeBlue) ||
    biteOther(snakeBlue, snake)
  ) {
    console.log("LOST");
  } else {
    setTimeout(function () {
...
function xOut(x) {
  return (
    x + radius < cRect.left - offsetX ||
    x + radius > cRect.right - offsetX ||
    x - radius < cRect.left - offsetX ||
    x - radius > cRect.right - offsetX
  );
}
function yOut(y) {
  return (
    y + radius < cRect.top - offsetY ||
    y + radius > cRect.bottom - offsetY ||
    y - radius < cRect.top - offsetY ||
    y - radius > cRect.bottom - offsetY
  );
}
...
function biteOther(snake1, snake2) {
  ...
  for (let i = 0; i < snake2.snake.length; i++) {
    ...
    if (snake1.collision(bodyPart.pos)) return true;
  }
